...
func casting(particle,central,impulse):
	if get_node("../Hero/AnimatedSprite")
	var current_pos = Vector2(-30,-25) + CHAR_POS
	CENTRAL = impulse
	IMPULSE = Vector2(impulse.x,impulse.y + current_pos.direction_to(to_local(mouse_pos)).y*5 )
