...
var i = {"right" : 0, "left" : 0, "jump" : 0, "cast" : 0, "aoe" : 0,"aura" : 0}
...
#		yield($AnimatedSprite,"animation_finished")
	...
	if (Input.is_action_just_pressed("ui_aoe"))  and cast_timer.get_time_left() == 0:
		cast_timer.start()
		i.aoe = 1
	if (Input.is_action_just_pressed("ui_aura"))  and aura_timer.get_time_left() == 0:
		aura_timer.start()
		i.aura = 1
...
func _physics_process(_delta):
	...
	if velocity.y < 0 and jumping and in_anim == 0:
		$AnimatedSprite.play("jump")
	if velocity.y > 20 and is_on_floor() == false and in_anim == 0:
